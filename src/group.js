...
cmd({
        ...
        async(Void, citel, text) => {
          ...
          var role = "جيد✨";
            ...
            const profile = `*┏━ ╼━━╃⌬〔🥀〕⌬╄━━╾ ━┓*
 *⌝مــرحبـــآ╎${citel.pushName}🪄⌞*
*هہذهہ مـعلومـآت بــروفــآيلگ*
*⎔⋅• ━ ╼╃ ⌬〔🥀〕⌬ ╄╾ ━ •⋅⎔*
*❍↵┆الاسـمـ👤『${citel.pushName}』*

*❍↵┆البــآيو📝『${bioo}』*

*❍↵┆المـســتوى📊『${userq.level}』*

*❍↵┆الــــرول🧩『 ${role}』*

*❍↵┆عدد الرســائل✉️『${ttms}』*
*⎔⋅• ━ ╼╃ ⌬〔🥀〕⌬ ╄╾ ━ •⋅⎔*
*~⌬ تــ✍︎ــوقــيــع ↡~*
*~⟦ساكوراجي بوت⟧~*
*┗━ ╼━━╃⌬〔🥀〕⌬╄━━╾ ━┛*
        ...
        async(Void, citel, text) => {
          ...
          var role = "جيد✨";
            if (lvpoints <= 2) {
                ...
                var role = "🏆سوبر في الاس";
            } else if (lvpoints <= 36) {
                var role = "💍سوبريم واحد";
            } else if (lvpoints <= 38) {
                var role = "💎سوبريم اثنين";
            } else if (lvpoints <= 40) {
                var role = "🔮سيد السوبربم";
            } else if (lvpoints <= 42) {
                var role = "🛡اسطوري";
            } else if (lvpoints <= 44) {
                var role = "🏹اسطوري خارق";
            } else if (lvpoints <= 46) {
                var role = "⚔سوبر اسطوري";
            } else if (lvpoints <= 55) {
                var role = "🐉خالد";
            ...
            let ttms = `${userq.xp}` / 8;
            textr += `*❍↵┆الــــرول🧩『 ${role}』*\n *❍↵┆الإكــــس بي🛡『 ${userq.xp} / ${Levels.xpFor(
    userq.level + 1)}』*  \n*❍↵┆المـســتوى📊『${userq.level}』*\n*❍↵┆عدد الرســائل✉️『${ttms}』*\n *✦ ─┷┷┷┷ ⋅⊱✿⊰⋅ ┷┷┷┷─ ✦*`;
            try {
...
cmd({
    ...
    async(Void, citel, text) => {
        ...
        try {
            let users = citel.mentionedJid[0] ? citel.mentionedJid[0] : citel.quoted ? citel.quoted.sender : text.replace(/[^0-9]/g, "") + "@s.whatsapp.net";
            if (!users) return;
            await Void.groupParticipantsUpdate(citel.chat, [users], "تم سحب الاشراف");
        } catch {
            //		citel.reply(tlang().botAdmin);
